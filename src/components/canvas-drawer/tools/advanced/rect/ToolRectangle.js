...
import { createRectHandlerSingleton } from './RectHandler';
...
const ToolRectangle = (props) => {
    ...
    React.useEffect(()=>{
        console.log("Tool rectangle was loaded successfully.");
    }, []);

    React.useEffect(()=>{
        if(context !== undefined) { 
            setHandler(createRectHandlerSingleton(context.mainContext, context.tempContext, selected));
        }
    }, [context]);
    ...
    const addMeventListener = () => {
        ...
        dispatcher.addListener(MEVENT_KINDS.SELECTED_SHAPE, (mevent) => {
            if(mevent.value.shape !== 'rect') setSelected(false);
