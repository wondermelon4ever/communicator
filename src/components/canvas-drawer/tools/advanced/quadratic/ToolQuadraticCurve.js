...
import { createQuadraticHandlerSingleton } from './QuadraticHandler';
...
const ToolQuadraticCurve = (props) => {
    ...
    React.useEffect(()=>{
        if(context !== undefined) { 
            setHandler(createQuadraticHandlerSingleton(context.mainContext, context.tempContext, selected));
        }
    }, [context]);
    ...
    const addMeventListener = () => {
        ...
        dispatcher.addListener(MEVENT_KINDS.SELECTED_SHAPE, (mevent) => {
            if(mevent.value.shape !== 'quadratic-curve') setSelected(false);
